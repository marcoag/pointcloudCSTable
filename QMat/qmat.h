...
namespace RMat
{
	...
	{
	public:
		DataBuffer(const int size): QSharedData()
		{
			dataSize = size;
			data = new T[dataSize];
		}
		DataBuffer(const DataBuffer &tensorData): QSharedData()
		{
			dataSize = tensorData.dataSize;
			data = new T[dataSize];
			memcpy(getWriteData(), tensorData.getReadData(), dataSize*sizeof(T));
		} 
